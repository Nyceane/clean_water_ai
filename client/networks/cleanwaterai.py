...
helium.connect()
channel = helium.create_channel(b'Clean Water AI')
...
NETWORK_STAT_TXT = "./WaterNet/stat.txt"    # stat.txt for networ
NETWORK_CATEGORIES_TXT = "./WaterNet/categories.txt" # categories.txt for network
...
def postprocess(output):
    ...
    global detection
    if detection != gNetworkCategories[order[last-i]]:
        datastring='confidence={0:.2f}'.format(output[order[last-0]]*100) + '&detection=' + gNetworkCategories[order[last-i]]
        print(datastring)
        channel.send(datastring.encode())
        detection = gNetworkCategories[order[last-i]]
